...
#how much is the offset between physics step inputs and calls from physics step scripts
const _physics_step_offset:int = -1
...
func which_device_just_pressed(actionName:StringName)->int:
	...
	if Engine.is_in_physics_frame():
		...
		for state in action.states:
			if state.just_pressed_physics == Engine.get_physics_frames()+_physics_step_offset:
				return state.device
...
func is_action_pressed(actionName:StringName, device:int=DeviceIndex.ACTIVE_OR_ANY)->bool:
	var action:InputtyAction = inputMap.getAction(actionName)
	if !action or !action.validState(device):
		return false
...
func is_action_just_ticked(actionName:StringName, device:int=DeviceIndex.ACTIVE_OR_ANY)->bool:
	...
	if Engine.is_in_physics_frame():
		#physics process check
		if deviceState.just_pressed_physics == Engine.get_physics_frames()+_physics_step_offset:
			return true
		if _physicsTick>=0.0:
			return false
...
func is_action_just_pressed(actionName:StringName, device:int=-2)->bool:
	...
	if Engine.is_in_physics_frame():
		#physics process check
		return action.getActionState(device).just_pressed_physics == Engine.get_physics_frames()+_physics_step_offset
...
func is_action_just_released(actionName:StringName, device:int=-2)->bool:
	...
	if Engine.is_in_physics_frame():
		#physics process check
		return action.getActionState(device).just_released_physics == Engine.get_physics_frames()+_physics_step_offset
...
func get_axis(negativeAction:StringName, positiveAction:StringName, deadzone:float=-1.0, device:int=-2)->float:
	...
	if action_neg.validState(device):
		retVar-=action_neg.getActionState(device).raw_strength
	if action_pos.validState(device):
...
func _pickIconSet(device:int, n:String=""):
	...
	if !Input.is_joy_known(device):
		_resources.controllerIconSets[0]
	...
	if event is InputEventMouseButton:
		...
		if event is InputEventWithModifiers:
			if event.ctrl_pressed:
				retStr = OS.get_keycode_string(Key.KEY_CTRL)+"+"
			if event.alt_pressed:
				retStr += OS.get_keycode_string(Key.KEY_ALT)+"+"
			if event.shift_pressed:
				retStr += OS.get_keycode_string(Key.KEY_SHIFT)+"+"
			if event.meta_pressed:
				retStr += OS.get_keycode_string(Key.KEY_META)+"+"
