			...
			if action:
				...
				for i in inputsSize:
					...
					if typ=="MouseButton":
						...
						if loadedInput.axis_value!=0.0:
							loadedInput.axis_value = sign(loadedInput.axis_value)
						loadedInput.device = -1
					else:# typ=="NULL":
						loadedInput = null
						pass
						
					if loadedInput is InputEventWithModifiers:
						...
						loadedInput.command_or_control_autoremap = config.get_value(loadedSection, "cmdctrlremap_"+str(i), false)
