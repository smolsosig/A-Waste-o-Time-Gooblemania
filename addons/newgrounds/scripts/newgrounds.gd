...
const keys_loc: String = "res://addons/newgrounds/keys.gd"
func _ready():
	if !ResourceLoader.exists(keys_loc):
		return
	
	get_node("%GameKeys").set_script(load(keys_loc))
	...
	app_id		= get_node("%GameKeys").blehhh_no_keys_for_you["app_id"]
	aes_key		= get_node("%GameKeys").blehhh_no_keys_for_you["aes_key"]
