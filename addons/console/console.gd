...
var enabled := true
var enable_on_release_build := true : set = set_enable_on_release_build
...
func _ready() -> void:
	...
	add_command("info", info, 0, 0, "Information regarding the game and this console.")
	add_command("map", change_level, ["level filename"], 1, "Changes the current level.")
	add_command("reset", reset, 0, 0, "Resets the level.")
	add_command("playmusic", play_music, ["music filename"], 1, "Plays music.")
...
#region AWoT commands
var no_main_menu: String = "You... know you can't really do that in the main menu, right? Right?"
...
func change_level(map_name: String) -> void:
	var check: bool
	check = ResourceLoader.exists("res://scenes/game/levels/%s.scn" % map_name) if SignalBus.desktop else ResourceLoader.exists("res://web/scenes/game/levels/%s.scn" % map_name)
	if check:
		SignalBus.emit_signal("level_load", map_name)
		print_smth("Requesting to load %s via console. Hey, you're smart." % map_name)
		toggle_console()
	else:
		Console.print_error("Level is non-existent, dumbshit.")

func reset() -> void:
	if SignalBus.game_mode == 2:
		SignalBus.emit_signal("reset")
		print_smth("Resetting level via console.")
		toggle_console()
	elif SignalBus.game_mode == 1:
		print_smth(no_main_menu)

func play_music(filename: String) -> void:
	var check := ResourceLoader.exists("res://assets/sounds/music/%s.ogg" % filename)
	if check:
		MusicManager.play_music(filename + ".ogg")
		print_smth("Now playing: %s.ogg." % filename)
	else:
		print_error("Music file doesn't exist, dumbshit.")

func stop_music() -> void:
	MusicManager.play_music("null.ogg")

func die() -> void:
	PlayerVar.health = 1
	PlayerVar.target_health = 0
	print_smth("May she rest in peace.")
	toggle_console()

func return_dialog(key:String) -> void:
	if key:
		if Staglobals.return_dialogue("it's rude to stare, y'know") != Staglobals.end_dialogue_signal:
			print_smth("%s returned \"%s\"." % [key, Staglobals.return_dialogue(key)])
		else:
...
func current_spawn(spawn_num: String) -> void:
	...
	Staglobals.current_spawn = int(spawn_num)
	print_smth("Current spawn now set from %s to %s." % [prior_spawn, spawn_num])

#func char_thealth(target_health: int, health_speed: float) -> void:
func thealth(target_health: String, health_speed: String) -> void:
	PlayerVar.target_health = int(target_health)
	PlayerVar.health_speed = float(health_speed)
	print_smth("Target health set to %s with speed of %s." % [PlayerVar.target_health, PlayerVar.health_speed])
...
func print_smth(text: Variant, print_godot := false) -> void:
	if not text is String:
		text = str(text)
	print_line("	   %s" % text, print_godot)
...
func help() -> void:
	rich_label.append_text("	[b]:::::::::::::::: CONSOLE COMMANDS :::::::::::::::[/b]\n")
	commands_list()
	rich_label.append_text("	[b]:::::::::::::::: CONSOLE CONTROLS :::::::::::::::[/b]:
...
func calculate(command : String) -> void:
	...
	if not expression.has_execute_failed():
		print_smth("%s. You guys know that calc is short for calculator? I'm just using slang, guys." % str(result))
	else:
		print_error("%s" % expression.get_error_text())
